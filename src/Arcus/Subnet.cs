...
namespace Arcus
{
                          ...
                          IComparable
    {
        ...
        /// </summary>
        private static Regex _ipv4OctetPartialRegex = new Regex(Ipv4OctetPartialPattern, RegexOptions.Compiled | RegexOptions.CultureInvariant);

        /// <summary>
        ///     Regex for rough shape of a subnet string
        /// </summary>
        private static Regex _roughSubnetRegex = new Regex(RoughSubnetStringPattern, RegexOptions.Compiled | RegexOptions.CultureInvariant | RegexOptions.IgnoreCase);

        /// <summary>
        ///     the number of usable addresses in the subnet (ignores Broadcast and Network addresses)
        /// </summary>
        public BigInteger UsableHostAddressCount => Length >= 2
                                                        ? Length - 2
                                                        : 0;
                                        ...
                                        IFormatProvider formatProvider)
        {
            ...
            switch (format?.Trim())
            {
                ...
                case "F":
                    return IsSingleIP
                               ? $"{this.NetworkPrefixAddress}"
                               : $"{this.NetworkPrefixAddress}/{this.RoutingPrefix}";
        ...
        public Subnet([NotNull] IPAddress lowAddress,
                      [NotNull] IPAddress highAddress)
            : base(CtorFactory(lowAddress, highAddress))
        {
            ...
            this.Netmask = IsIPv4
                               ? result.Mask
                               : null; // only set netmask for IPv4 based subnets
        }

        private static AddressTuple CtorFactory(IPAddress lowAddress,
                                                IPAddress highAddress)
        {
            ...
            {
                throw new ArgumentException($"{nameof(lowAddress)} and {nameof(highAddress)} must have matching address families");
            }

            if (!IPAddressUtilities.ValidAddressFamilies.Contains(lowAddress.AddressFamily))
            {
                throw new ArgumentException($"{nameof(lowAddress)} must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(lowAddress));
            }

            if (!IPAddressUtilities.ValidAddressFamilies.Contains(highAddress.AddressFamily))
            {
                throw new ArgumentException($"{nameof(highAddress)} must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(highAddress));
            }
        ...
        public Subnet([NotNull] IPAddress address,
                      int routingPrefix)
        ...
        private static AddressTuple CtorFactory([NotNull] IPAddress address,
                                                int routingPrefix)
        {
            #region Defense
            ...
            {
                throw new ArgumentException($"{nameof(address)} must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(address));
            }
            ...
            if (address.IsIPv4()
                && routingPrefix > IPAddressUtilities.IPv4BitCount)
            {
                throw new ArgumentOutOfRangeException(nameof(routingPrefix));
            }

            if (address.IsIPv6()
                && routingPrefix > IPAddressUtilities.IPv6BitCount)
            {
                throw new ArgumentOutOfRangeException(nameof(routingPrefix));
            }
        ...
        {
            this.RoutingPrefix = IsIPv4
                                     ? IPAddressUtilities.IPv4BitCount
                                     : IPAddressUtilities.IPv6BitCount;
            ...
            var netmaskBytes = Enumerable.Repeat((byte) 0xff, IPAddressUtilities.IPv4ByteCount)
                                         .ToArray();
        ...
        public static Subnet FromNetMask([NotNull] IPAddress address,
        ...
        public static bool TryFromNetMask(IPAddress address,
                                          IPAddress netmask,
                                          out Subnet subnet)
        {
            try
...
#pragma warning restore CA1031
                                       ...
                                       [NotNull] byte[] highAddressBytes)
        {
            ...
            {
                throw new ArgumentException($"could not convert {nameof(lowAddressBytes)} to an IPAddress", nameof(lowAddressBytes), e);
            }
            ...
            {
                throw new ArgumentException($"could not convert {nameof(highAddressBytes)} to an IPAddress", nameof(highAddressBytes), e);
            }
...
#pragma warning restore CA1031
        ...
        public static Subnet Parse([NotNull] string subnetString)
        {
            ...
            var matches = _roughSubnetRegex.Matches(subnetString);
            ...
            var addressString = match.Groups[1]
                                     .Value;
            var routePrefixString = match.Groups[2]
                                         .Value;
...
#pragma warning restore CA1031
                                   ...
                                   int routingPrefix)
        {
            ...
            if (!IPAddressUtilities.ValidAddressFamilies.Contains(address.AddressFamily))
            {
                throw new ArgumentException($"must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(addressString));
            }
        ...
        /// <param name="subnet">the created subnet or <see langword="null" /> on failure</param>
        /// <returns><see langword="true" /> on success</returns>
        public static bool TryParse(string addressString,
                                    int routingPrefix,
                                    out Subnet subnet)
        {
            try
...
#pragma warning restore CA1031
        ...
        public static Subnet Parse([NotNull] string lowAddressString,
                                   [NotNull] string highAddressString)
        {
            ...
            if (lowAddress.AddressFamily != highAddress.AddressFamily)
            {
                throw new ArgumentException($"{nameof(lowAddressString)} and {nameof(highAddressString)} must have matching address families");
            }

            if (!IPAddressUtilities.ValidAddressFamilies.Contains(lowAddress.AddressFamily))
            {
                throw new ArgumentException($"{nameof(lowAddressString)} must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(lowAddressString));
            }

            if (!IPAddressUtilities.ValidAddressFamilies.Contains(highAddress.AddressFamily))
            {
                throw new ArgumentException($"{nameof(highAddressString)} must have an address family equal to {string.Join(", ", IPAddressUtilities.ValidAddressFamilies)}", nameof(highAddressString));
            }

            if (!highAddress.IsGreaterThanOrEqualTo(lowAddress))
            {
                throw new InvalidOperationException($"{nameof(highAddressString)} must be greater or equal to {nameof(lowAddressString)}");
            }
        ...
        /// <param name="subnet">the created subnet or <see langword="null" /> on failure</param>
        /// <returns><see langword="true" /> on success</returns>
        public static bool TryParse(string lowAddressString,
...
#pragma warning restore CA1031
                                              ...
                                              out Subnet subnet)
        {
            if (string.IsNullOrWhiteSpace(input)
                || !_ipv4OctetPartialRegex.IsMatch(input))
            {
                subnet = null;
                return false;
            }
            ...
            subnet = Parse(input + string.Concat(Enumerable.Repeat(".0", IPAddressUtilities.IPv4OctetCount - octetCount)), octetCount * 8);
            return true;
        }
        ...
        /// <returns><see langword="true" /> on success</returns>
        [Obsolete("the needs for this method are very specialized and may not be what the developer is expecting; this is likely to be replaced by a host of other more explicit and useful methods")]
        public static bool TryIPv6FromPartial(string input,
                                              out IEnumerable<Subnet> subnets)
        {
                ...
                && address.IsIPv6()) // no collapse, but incomplete address
            {
                ...
                for (var permutation = 0; permutation <= permutationLimit; permutation++)
                {
                    ...
                    if (!IPAddress.TryParse(addressString + "::", out var subnetAddress)
            ...
            // chick input for multiple occurrences of discrete "::" substrings
            bool DoubleColonsAppearsMultipleTimes(string @in)
            {
                ...
                return @in.Length >= 4
                       ...
                       && @in.Substring(firstIndex + 2, @in.Length - firstIndex - 2)
                             .IndexOf(colons, StringComparison.Ordinal)
                       != -1;
            }
        }
        ...
        {
            public AddressMaskAndPrefixTuple([NotNull] IPAddress head,
                                             [NotNull] IPAddress tail,
                                             [NotNull] IPAddress mask,
                                             int prefix)
            {
        ...
        private static AddressMaskAndPrefixTuple NormalizeAndCreateNetMask([NotNull] IPAddress head,
                                                                           [NotNull] IPAddress tail)
        {
            ...
            int CalculateRoutingPrefix(byte[] hb,
                                       byte[] tb)
            {
        ...
        {
            public AddressAndMaskTuple([NotNull] IPAddress head,
                                       [NotNull] IPAddress tail,
                                       [NotNull] IPAddress mask)
            {
                this.Head = head ?? throw new ArgumentNullException(nameof(head));
        ...
        private static AddressAndMaskTuple NormalizeAndCreateNetMask([NotNull] IPAddress head,
                                                                     int routingPrefix)
        {
            var headBytes = head.GetAddressBytes();
            var addressByteLength = headBytes.Length;

            var maskBytes = Enumerable.Repeat((byte) 0xFF, addressByteLength)
                                      .ToArray()
        ...
        public void Deconstruct([NotNull] out IPAddress networkPrefixAddress,
                                [NotNull] out IPAddress broadcastAddress,
