...
namespace Arcus
{
                                  ...
                                  ISerializable
    {
        ...
        public static bool TryCollapseAll(IEnumerable<IPAddressRange> ranges,
                                          out IEnumerable<IPAddressRange> result)
        {
            ...
            if (rangeList.Any(r => r.AddressFamily
                                   != rangeList.First()
                                               .AddressFamily))
            {
                result = Enumerable.Empty<IPAddressRange>();
            ...
            {
                var last = resultList.Last(); // take end of result to process over (is first in first iteration)
                                         ...
                                         out IEnumerable<IPAddressRange> result)
        {
            ...
            foreach (var exclusion in excludedRangesList)
            {
                ...
                if (exclusion.Contains(initialRange.Tail))
                {
                    //resultList[lastIndex].Tail = exclusion.Head.Increment(-1);
                    var head = resultList[lastIndex]
                        .Head;
                ...
                if (exclusion.Contains(last.Head))
                {
                    ...
                    //resultList[lastIndex].Head = exclusion.Tail.Increment();
                    var head = exclusion.Tail.Increment();
                    var tail = resultList[lastIndex]
                        .Tail;
                    resultList[lastIndex] = new IPAddressRange(head, tail);
                ...
                // resultList[lastIndex].Tail = exclusion.Head.Increment(-1);
                resultList[lastIndex] = new IPAddressRange(resultList[lastIndex]
                                                               .Head,
                                                           exclusion.Head.Increment(-1));
